...
public abstract class BasePlugin extends JavaPlugin{
	protected final static String selfPluginName=ClojurePlugin.class.getPackage().getName();//"craftyrepl";
...
//	private final static Logger logger=Logger.getLogger( "Minecraft" );//this is equivalent to above
	...
	static {//static initializer block
		...
		//this should only be executed for craftyrepl(the main not any children) plugin, and it is so if children have a depend on craftyrepl
		//bukkit will then make sure craftyrepl is loaded before them
...
//				isServerReloaded()?clojure.lang.RT.T:clojure.lang.RT.F
	...
	public final static boolean isServerReloaded() {
		...
		//or do we simply give no support for the variant of `plugman load craftyrepl` into an already running server(after just having placed the .jar first time in plugins folder)?
	...
	public final void onLoad() {
		...
		if (!getName().equals(selfPluginName)) {
			//we don't have to add craftyrepl.jar to classpath here
    ...
    public InputStream getResource( String fileName ) {
    	...
    	//or else it will get the file from craftyrepl not from memorystone, ie. file name like config.yml which exists in both in same location
    ...
    public static final void info(Class cls, String msg) {
    	String className = cls.getName();//we won't know the difference if we're in main or child plugins (craftyrepl or memorystone) because they both use the same main class to start
